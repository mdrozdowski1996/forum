...
public class ApplicationController implements Initializable {
	...
	private Dialog<NodeViewModel> createAddDialog() throws ApplicationException {
		...
		/*return new Dialog<NodeViewModel>() {
			{
				ButtonType comment = new ButtonType("dupa");
				getDialogPane().setContent(new Label(""));
				getDialogPane().getButtonTypes().addAll(comment, ButtonType.OK, ButtonType.CANCEL, ButtonType.APPLY);
				
				setResultConverter(buttonType -> 
						buttonType == ButtonType.OK ? new CommentViewModel("Some text", "Anonymous") :
							(buttonType == comment ? new SurveyViewModel("admin1", "comment") : null));
			}
		};*/
	}
